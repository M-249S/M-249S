<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:2563EB,100:0F172A&height=200&section=header&text=Mujahid%20Suliman&fontSize=50&fontColor=ffffff&animation=fadeIn&fontAlignY=38" />

### Python Backend Engineer • Building AI Products

<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&weight=700&size=28&duration=3000&pause=800&center=true&vCenter=true&width=850&lines=Python+Backend+Engineer;Building+Production+AI+Systems;FastAPI+%7C+Python+%7C+PostgreSQL;Founder+of+ResumeIQ;Building+in+Public+%F0%9F%9A%80" />

Building scalable backend systems with Python, FastAPI, and Generative AI.

</div>

---

## 🚀 About Me

I'm a Python Backend Engineer focused on building production-ready, AI-powered applications — not demos. I specialize in scalable backend architecture, REST API design, authentication systems, and integrating LLMs into real products people actually use.

Currently building **[ResumeIQ](https://resumeiq.dev)**, an AI-powered resume optimization platform that helps job seekers improve their ATS scores, rewrite their resumes, and generate tailored cover letters.

> Learn → Build → Share

---

## 🎯 What I'm Focused On

- 🚀 Shipping **ResumeIQ** as a production SaaS
- ⚡ Backend systems with FastAPI and PostgreSQL
- 🧠 LLMs, RAG, and prompt engineering
- 📚 Building in public and sharing what I learn
- 💼 Open to backend and AI integration freelance work

---

## ⚡ Tech Stack

**Languages**
<img src="https://skillicons.dev/icons?i=python,js,html,css" />

**Backend**
<img src="https://skillicons.dev/icons?i=fastapi,flask,nodejs" />

**Database**
<img src="https://skillicons.dev/icons?i=postgresql,mysql,sqlite" />

**Cloud & DevOps**
<img src="https://skillicons.dev/icons?i=docker,linux,git,github,nginx" />

**AI / LLM**
Google Gemini · OpenAI · Prompt Engineering · LLM Applications · Retrieval-Augmented Generation (RAG)

---

## 🏆 Featured Project — ResumeIQ

AI-powered resume optimization platform.

| Feature | Description |
|---|---|
| ✅ ATS Resume Analysis | Scores resumes against job descriptions |
| ✅ Resume Rewriting | AI-generated improvement suggestions |
| ✅ Cover Letter Generator | Tailored, role-specific letters |
| ✅ Resume Matching | Matches resume content to job requirements |
| ✅ Authentication & Dashboard | Secure user accounts with resume history tracking |

**Stack:** FastAPI · PostgreSQL · JWT Auth · Google Gemini AI · React

---

## 💼 Available For

| Service | Status |
|---|---|
| Backend Development | ✅ Available |
| FastAPI REST APIs | ✅ Available |
| AI Integration | ✅ Available |
| SaaS MVP Development | ✅ Available |
| Python Automation | ✅ Available |

---

## 📊 GitHub Stats

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=M-249S&show_icons=true&theme=tokyonight&hide_border=true&cache_seconds=1800"/>
<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=M-249S&layout=compact&theme=tokyonight&hide_border=true&cache_seconds=1800"/>

<img src="https://streak-stats.demolab.com?user=M-249S&theme=tokyonight&hide_border=true"/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=M-249S&theme=tokyo-night&hide_border=true"/>

</div>

---

## 🐍 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/M-249S/M-249S/output/github-contribution-grid-snake.svg" />

</div>

> Updates automatically via the `snake.yml` GitHub Action on every contribution.

---

## 🌍 Connect With Me

<p align="center">
<a href="https://www.linkedin.com/in/mujahid-suliman-6a9875425/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
<a href="https://x.com/MujahidCode"><img src="https://img.shields.io/badge/X-000000?style=for-the-badge&logo=x&logoColor=white"/></a>
<a href="https://github.com/M-249S"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/></a>
<a href="https://www.reddit.com/user/Remote_Singer_6287/"><img src="https://img.shields.io/badge/Reddit-FF4500?style=for-the-badge&logo=reddit&logoColor=white"/></a>
<a href="https://resumeiq.dev"><img src="https://img.shields.io/badge/Portfolio-ResumeIQ-0A66C2?style=for-the-badge"/></a>
</p>

<p align="center">
<img src="https://komarev.com/ghpvc/?username=M-249S&label=Profile%20Views&color=0A66C2&style=for-the-badge"/>
</p>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0F172A,100:2563EB&height=150&section=footer&animation=fadeIn" />

<div align="center">

**Building AI products in public.**
Learn → Build → Share

</div>
